...
- **Latest merged:** PR #52 (increment 24 — Web credentials write-path). `main` is clean; **no open PRs**; no in-flight branches.
- **Immediate next:** **increment 24.5 — Web: UX revamp** (decided with the user). NOT feature work — a **design-led rethink** of the dashboard: move it from "management portal" toward a **normal-user / Composio-like connect-once experience** (information architecture, onboarding/empty states, the connect flow, approachability for a non-power-user) + a designer's-eye polish + an a11y pass across all pages. Method file `24.5-web-ux-revamp.md` to be written. Build on the inc-23 design system + the inc-24 mutation pattern (the write-paths now exist to wire flows to). Reviewers: `junction-web-reviewer` + design/impeccable skills + `junction-web-verify` browser pass.
- **The plan lives in** `docs/methods/README.md` (the increment map; 24.5 then 25→31) and `docs/futures/revisit-when.md` (Tier-2 trigger-deferred).
