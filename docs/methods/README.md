...
| 26 | Web: remaining profile/source work — toolFilter **edit-in-place** (needs a thin core op: extend the source-ref repo `SourceOp` with `setFilter`) + the junction-keys / single-endpoint auth backend direction. (Profile add/remove/toggle/new/delete + the add-time credential picker shipped in inc 25; what remains is in-place filter editing + the keys backend.) | — | planned |
