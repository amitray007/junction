...
| 29 | OAuth vault (arctic) — connect OAuth platforms; token refresh | — | planned |
| 30 | Audit (pino) — structured tool-call / credential-use log (consumes inc-27 per-key identity for attribution) | — | planned |
| 31 | Security & ops hardening — vault backup/recovery + master-key rotation + tool-poisoning mitigation + deferred CI security tooling (knip, type-coverage, semgrep, CodeQL, secret-scan, SPDX CI gate) | — | planned |
| 32 | Code-mode — QuickJS-WASM over the `ToolProvider` proxy (the fast execution path; "base solid" trigger) | — | planned |
| 33 | **Distribution — publish `junction` + the `junction install` story.** Deliberately the LAST Tier-1 increment (moved from 29, user decision 2026-07-03): **do not publish until the full product is proven working locally end-to-end** — connect-once via OAuth (29) + audit (30) + hardening (31) done and **dogfooded against the real running junction**, not just green gates. Publishing is irreversible-ish (npm name, users depend on it), so it's earned, not scheduled early. Brings the npm publish path + `publint`/`attw` packaging gates + the bin/exports story + a decide-what-ships call (`@junction/web` bundled vs separate). | — | planned (gated on local-proof) |
