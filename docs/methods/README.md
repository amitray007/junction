...
| 25 | **Web UX v2** — user-driven IA + functionality revision (13 items). Theme→light/dark only; sidebar groups ([Dashboard, Settings]·[Platforms, Profiles, Credentials], no "Manage" label); badge/table/empty-table patterns adapted from the design reference; dashboard rethink (Connect-an-Agent hero, aligned At-a-Glance, subtle ComingSoon, no card-in-card); **Settings page + real MCP host** (config `mcpHost` + `JUNCTION_MCP_HOST` env); **Credentials → flat paginated table + group-dividers** (Variant C); **Profiles → master-detail + route editing** (add/remove/toggle/new/delete; filter-edit is ComingSoon — no core op); fix the Credentials ⋯ click bug. Pulls inc-26 profile mutations forward. | `25-web-ux-v2.md` | written |
| 26 | Web: remaining profile/source work — toolFilter **edit-in-place** (needs a thin core op) + the junction-keys / single-endpoint auth backend direction | — | planned |
| ~ | Platform management (web add/edit/delete + extract platform add/refresh orchestration `cli → core`) — was the old "inc 25"; reslot after 25/26 | — | planned |
