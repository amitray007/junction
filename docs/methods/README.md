...
| 27 | **junction-keys / single-endpoint MCP auth** ⚠️ revises a core invariant. Mint per-profile / global / multi-profile API keys (scoped, revocable, hashed-at-rest); a **single shared MCP endpoint** authenticates by key and the **key selects the profile(s)** — replacing today's unauthenticated per-profile `/profiles/{name}/mcp`. Keys table + **auth middleware in `mcp/server`** + web mint/scope/revoke UI + per-key identity (the attribution audit (31) will consume). When built: update CLAUDE.md + the design spec (the "per-profile endpoints, not shared-endpoint filters" invariant) + the AgentConfig "Shared endpoint" ComingSoon. (Split out of the old inc 26.) | `27-junction-keys-single-endpoint.md` | written |
