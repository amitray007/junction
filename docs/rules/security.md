...
- **Containment is validated for BOTH backends.** `validatePolicy` refuses a policy where any granted read/write path is an ancestor of a credential/secret dir (would pull secrets into the sandbox), and where `cwd` is not within a granted path. bwrap has no deny primitive, so this ancestor check is its only confidentiality guard.
