...
- **No secrets in sandbox:** spawn with an EXPLICIT `env:` object from `policy.env` only — never inherit `process.env`. Validate `policy.env` at the boundary: reject any key matching the `JUNCTION_` reserved prefix (`JUNCTION_MASTER_KEY`, `JUNCTION_MASTER_KEY_FILE`, `JUNCTION_HOME`, any future `JUNCTION_*`) or the dynamic-linker/interpreter denylist class (`LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_AUDIT`, `DYLD_*`, `NODE_OPTIONS`) → `policy-invalid` (shared `sandbox/env-denylist.ts` predicate, consumed by `validatePolicy`, `CliConnectionSchema`'s `credentialEnvVar` refine, and `McpConnectionSchema`'s stdio env refine). Deliberately does NOT reject `*_TOKEN`/`*_SECRET`/`*_KEY`-suffixed names (inc 41 Fable ruling) — that suffix heuristic blocked legitimate credential names (e.g. `GH_TOKEN`, the only var `gh` reads) while adding no real protection: the injected value is always the user's own store-resolved credential, and sandbox env is an explicit allowlist that never inherits `process.env`. The CredentialStore is NEVER passed to a sandbox.
