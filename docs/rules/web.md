...
- **Inc-28 exception (provider libs as server-only runtime deps).** `@junction/source-runtime`, `@junction/mcp-client`, `@junction/openapi-client`, and `@junction/graphql-client` **MAY** be listed in `packages/web/package.json` `dependencies` — but ONLY because `@junction/source-runtime`'s `buildProvider` does `await import("@junction/mcp-client")` (etc.) at **runtime**, and under pnpm's isolated `node_modules` that lazy import resolves from web ONLY if web depends on them directly (a transitive dep is not reachable). They **MUST NOT** be *statically imported* anywhere in `packages/web/src` (they are reached only through `source-runtime` inside `probe.server.ts`), **MUST** be in `vite.config.ts` `ssr.external` + `optimizeDeps.exclude` (server-only), and are `web:leakcheck`-denied from the client bundle. The runtime-resolution requirement is `web:smoke`-gated (a missing direct dep → empty probe/call tool list — inc 28).
