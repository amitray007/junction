...
6. **Background review** — dispatch the warranted reviewers **in parallel as background agents**; they may finish without surfacing findings, so **ping them for their report**, then consolidate and fix. Treat review as adversarial, not a rubber stamp — **expect, and hunt for, at least one real finding**.
   ...
   → **An independent external AI code-review tool**, if one is configured, as an extra net over the agents above. Triage its findings like any reviewer's (verify, don't rubber-stamp). _Currently configured: CodeRabbit CLI (`coderabbit review --agent --base main`; `--type committed|all`; `coderabbit review findings` to re-show)._ Swap the command if the tool changes; the step is "run whatever external reviewer is set up," not tool-specific.
7. **Ask the user to test.**
8. **→ USER APPROVES (gate)** → commit & next increment.
   → **Before merging:** run the configured external AI reviewer on the branch and address any real findings (user-requested pre-merge check, 2026-07-03; currently CodeRabbit — `coderabbit review --agent --base main`).
