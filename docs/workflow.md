...
6. **Background review** — dispatch the warranted reviewers **in parallel as background agents**; they may finish without surfacing findings, so **ping them for their report**, then consolidate and fix. Treat review as adversarial, not a rubber stamp — **expect, and hunt for, at least one real finding**.
   ...
   → **An independent external AI code-review tool**, if one is configured, as an extra net over the agents above. Triage its findings like any reviewer's (verify, don't rubber-stamp). _No external AI reviewer is currently configured (CodeRabbit was removed, user decision 2026-07-06)._ The step is "run whatever external reviewer is set up," not tool-specific — it is a no-op until one is configured again.
7. **Ask the user to test.**
8. **→ USER APPROVES (gate)** → commit & next increment.
   → **Before merging:** if an external AI reviewer is configured, run it on the branch and address any real findings. _None is currently configured (CodeRabbit removed, user decision 2026-07-06)_ — so the merge gate is: CI green + the review-thread-resolution ruleset satisfied (resolve any human/tool review threads that exist).
