...
| 2026-06-29 (24.6) | ~~**Two-column Dashboard**~~ (superseded by inc-25) | Flat equal-weight stack had no hierarchy; the most important block (Connect) didn't read as the point |
...
| 2026-06-29 (25) | **Theme = light/dark only** (drop "system"); default dark; first-paint seeds from `prefers-color-scheme` once (no localStorage write until explicit toggle) | User decision; explicit two-state toggle is clearer than a 3-way cycle; no-flash + OS-pref-honored-once preserved |
| 2026-06-29 (25) | **Sidebar = two groups** [Dashboard, Settings] · [Platforms, Profiles, Credentials], no "Manage" eyebrow | User decision; the mono group-eyebrow was slop-adjacent; a hairline/gap separates groups quietly |
| 2026-06-29 (25) | **Badge** = dot + label, `color-mix` tinted bg (12%) + 1px border (30%), fixed height; **`neutral`** count-chip variant (no dot) distinct from status | Reference-pattern adoption mapped to our blue/Geist tokens; a count next to a title must not read as a "Configured" status |
| 2026-06-29 (25) | **Single-column Dashboard**: Connect-an-Agent full-width hero → aligned 3-cell At-a-Glance strip → quiet System → ComingSoon footer (revisits the 24.6 2-col) | Connect's config is wide and benefits from full width; the equal-cell stat strip + full-width hero solve the "dead right half" without a 2-col grid; one container per block (killed the card-in-card) |
| 2026-06-29 (25) | **Settings page + a real MCP host** (config `mcpHost` + `JUNCTION_MCP_HOST` env, config wins) | The host makes Connect-an-Agent's config real + copyable — but the "shared HTTP endpoint isn't live yet, today stdio" note stays (still stdio-only; no HTTP transport) |
| 2026-06-29 (25) | **Credentials = ONE flat paginated table** + platform group-divider rows (Variant C, chosen from 3 mockups) | Search/sort/pagination + a single scannable table beats N per-platform cards; Kind shows the TRUE stored kind (bearer only) |
| 2026-06-29 (25) | **Profiles = master-detail** (list ↔ detail); **route editing** is the first profile write-path | Add/remove/toggle route + new/delete profile over the existing core ops; **edit-tool-access = ComingSoon** (no filter-update core op — filters set at add-route time, shown read-only) |
| 2026-06-29 (25) | Row-action **⋯ trigger is always visible** (`opacity-40` → 100% on hover), never `opacity-0`-until-hover | The opacity-0 pattern made the ⋯ unclickable for pointer users (Radix opens on pointer events; keyboard worked, hiding the bug) — a primary row action must be discoverable + hittable |
