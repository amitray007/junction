...
async function runRekeySequence(
  ...
  try {
    ...
    if (tmpKeys.size !== expectedKeys.size || [...expectedKeys].some((k) => !tmpKeys.has(k))) {
      ...
      // nosemgrep: no-bare-throw-in-core -- caught by the enclosing try/catch below, converted to a typed Result
    ...
    for (const [secretRef, expectedPlaintext] of plaintextByRef) {
      ...
      // nosemgrep: no-bare-throw-in-core -- same enclosing try/catch as above, converted to a typed Result
