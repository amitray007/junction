...
export function ToolCard({
  ...
  return (
    <ToolCardShell
      name={tool.name}
      index={index}
      summary={summaryCommand(tool)}
      expanded={expanded}
      onToggle={onToggle}
      onRemove={onRemove}
      canRemove={canRemove}
      removeDisabledTitle="A CLI platform needs at least one tool"
    >
      <div className="flex gap-4">
        <Field id={`tool-${tool.key}-name`} label="Name" error={errors?.name} className="flex-1">
          <Input
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            <span style={{ fontSize: "var(--text-caption)", color: "var(--gray-900)" }}>
              ...
              positions share one arg name, or an argv slot references an undeclared arg). Edit the
              raw JSON instead.
            </span>
          </div>
                ...
                chips.map((chip, i) => (
                  <span
                    ...
                    style={{
                      ...
                      backgroundColor: chip.kind === "arg" ? "var(--blue-bg)" : "var(--gray-100)",
                    }}
                  >
                    {chip.label}
                  </span>
