...
4. **Fan out into parallel subagents; integrate serially.** Within an increment, dispatch **one Sonnet builder subagent per slice** (each from its self-contained method-file slice) — **not** worktrees. The slices land through a **serial integration** in the one working tree — core slice first, then leaves — running **`pnpm verify` after each** before the next is applied. That serial gate is what catches the one real danger of parallel builds (semantic conflicts that typecheck/merge cleanly but break together), and it makes worktrees unnecessary here: nothing writes to the tree simultaneously. Reviews run per-slice, in parallel (cheap, no human yet). **Worktrees (`isolation: "worktree"`) are reserved for *cross-increment* parallelism — deferred until `core` stabilizes** (see the deferred note in `docs/methods/_waves.md`); they earn their cost only when whole increments build simultaneously on divergent branches.
